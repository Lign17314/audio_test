/* Copyright 2025
 * Streaming FBank Extractor - 30ms Chunk Version with Threading
 * 
 * 功能：多线程流式FBank特征提取 + Encoder推理
 * 
 * 线程模型：
 * - 主线程（生产者）：读取音频，提取FBank特征，将特征帧放入队列
 * - 消费者线程：从队列中取出特征帧，进行encoder推理
 * 
 * 处理流程：
            ...
            if (should_infer) {
                ...
                if (right_context_idx >= 0) {
                    ...
                    if (rc_tensor) {
                        ...
                        if (rc_data) {
                            memcpy(rc_data, right_context_data.data(), 
                                   right_context_data.size() * sizeof(float));
                        }
                    }
                }
                ...
                if (input_tensor) {
                    float* input_ptr = tflite::GetTensorData<float>(input_tensor);
                    if (!input_ptr) {
                        int tensor_idx = subgraph->inputs()->Get(input_tensor_idx);
                        TfLiteEvalTensor* eval_tensor = interpreter.GetTensor(tensor_idx, 0);
                        if (eval_tensor && eval_tensor->data.raw) {
                            input_tensor->data.data = eval_tensor->data.data;
                            input_tensor->data.raw = eval_tensor->data.raw;
                            input_ptr = tflite::GetTensorData<float>(input_tensor);
                        }
                    }
                    if (input_ptr) {
                        memcpy(input_ptr, input_data.data(), input_data.size() * sizeof(float));
                    }
                }
                ...
                if (!output_data) {
                    printf("[Consumer] ERROR: Cannot access output tensor data\n");
                    return;
                }
                
                // 确定输出维度（第一次推理时）
                if (output_dim == 0 && output_tensor->dims && output_tensor->dims->size >= 3) {
                    output_dim = output_tensor->dims->data[2];
                    printf("[Consumer] Output dimension: %zu\n", output_dim);
                } else if (output_dim == 0) {
                    printf("[Consumer] WARNING: Cannot determine output dimension\n");
                    if (output_tensor->dims) {
                        printf("  dims->size = %d\n", output_tensor->dims->size);
                        for (int d = 0; d < output_tensor->dims->size; d++) {
                            printf("  dims->data[%d] = %d\n", d, output_tensor->dims->data[d]);
                        }
                    } else {
                        printf("  dims is NULL\n");
                    }
            ...
            while (!frame_buffer.empty()) {
                ...
                if (output_tensor) {
                    ...
                    if (output_data) {
                        ...
                        printf("[Consumer] ERROR: output_data is NULL!\n");
                    }
                } else {
...
int main(int argc, char* argv[]) {
    printf("========================================\n");
    printf("Streaming FBank - 30ms Threaded Version\n");
    printf("========================================\n\n");
    ...
    const char* model_file = "../../../tflite_models/fsmn_encoder_stateful_float32.tflite";
    const char* output_file = "streaming_fbank_30ms_threaded_logits.npy";
