...
bool GetQuantizationParams(const TfLiteTensor *tensor, float &scale, int32_t &zero_point)
{
    ...
    zero_point = (quant_params->zero_point && quant_params->zero_point->data) ? quant_params->zero_point->data[0] : 0;
    return true;
}
 ...
 */
bool SetInputTensor(TfLiteTensor *input_tensor, const tflite::Model *model, tflite::MicroInterpreter &interpreter,
                    int input_tensor_idx, const std::vector<float> &input_data, float input_scale,
 ...
 */
bool ReadOutputTensor(TfLiteTensor *output_tensor, const tflite::Model *model, tflite::MicroInterpreter &interpreter,
                      int output_tensor_idx, std::vector<float> &output_data, float output_scale,
 ...
 */
void consumer_thread(FeatureQueue *queue, std::vector<std::vector<float>> *output_logits,
                     std::atomic<size_t> *frame_count, const char *model_file)
{
    ...
    printf("[Consumer] Tensor arena size: %d bytes (%.2f MB)\n", kTensorArenaSize,
           kTensorArenaSize / (1024.0 * 1024.0));
    ...
    for (size_t i = 0; i < NUM_CACHE_LAYERS; i++)
    {
        ...
        if (cache_in_tensor)
        {
            cache_in_is_quantized[i] =
                GetQuantizationParams(cache_in_tensor, cache_in_scales[i], cache_in_zero_points[i]);
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            printf("[Consumer] ERROR: Cache[%zu] invalid index %d (inputs_size=%zu)\n", cache_idx,                     \
                   cache_input_indices[cache_idx], interpreter.inputs_size());                                         \
            return;                                                                                                    \
        }                                                                                                              \
        ...
        {                                                                                                              \
            ...
            {                                                                                                          \
                data[j] = QuantizeFloatToInt8(caches[cache_idx][j], cache_in_scales[cache_idx],                        \
                                              cache_in_zero_points[cache_idx]);                                        \
            }                                                                                                          \
        }                                                                                                              \
        else if (eval_tensor->type == kTfLiteInt16)                                                                    \
        {                                                                                                              \
            ...
            for (size_t j = 0; j < caches[cache_idx].size(); j++)                                                      \
            {                                                                                                          \
                data[j] = QuantizeFloatToInt16(caches[cache_idx][j], cache_in_scales[cache_idx],                       \
                                               cache_in_zero_points[cache_idx]);                                       \
            }                                                                                                          \
        }                                                                                                              \
        else if (eval_tensor->type == kTfLiteFloat32)                                                                  \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            {                                                                                                          \
                ...
                {                                                                                                      \
                    caches[cache_idx][j] =                                                                             \
                        DequantizeInt8ToFloat(data[j], cache_out_scales[cache_idx], cache_out_zero_points[cache_idx]); \
                }                                                                                                      \
            }                                                                                                          \
            else if (eval_tensor->type == kTfLiteInt16)                                                                \
            {                                                                                                          \
                ...
                for (size_t j = 0; j < caches[cache_idx].size(); j++)                                                  \
                {                                                                                                      \
                    caches[cache_idx][j] = DequantizeInt16ToFloat(data[j], cache_out_scales[cache_idx],                \
                                                                  cache_out_zero_points[cache_idx]);                   \
                }                                                                                                      \
            }                                                                                                          \
            else if (eval_tensor->type == kTfLiteFloat32)                                                              \
            {                                                                                                          \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            {                                                                                                          \
                ...
                {                                                                                                      \
                    output_vec[j] =                                                                                    \
                        DequantizeInt8ToFloat(data[frame_idx * output_dim + j], output_scale, output_zero_point);      \
                }                                                                                                      \
            }                                                                                                          \
            else if (eval_tensor->type == kTfLiteInt16)                                                                \
            {                                                                                                          \
                ...
                {                                                                                                      \
                    output_vec[j] =                                                                                    \
                        DequantizeInt16ToFloat(data[frame_idx * output_dim + j], output_scale, output_zero_point);     \
                }                                                                                                      \
            }                                                                                                          \
            else if (eval_tensor->type == kTfLiteFloat32)                                                              \
            {                                                                                                          \
    ...
    while (true)
    {
        ...
        if (queue->pop(frame))
        {
            ...
            if (should_infer)
            {
                ...
                {
                    memcpy(input_data.data() + i * FEATURE_DIM, frame_buffer[i].data(), FEATURE_DIM * sizeof(float));
                }
                ...
                {
                    memcpy(right_context_data.data() + i * FEATURE_DIM, frame_buffer[CHUNK_SIZE + i].data(),
                ...
                std::vector<std::vector<float>> remaining(frame_buffer.begin() + CHUNK_SIZE, frame_buffer.end());
    ...
    if (!frame_buffer.empty())
    {
        ...
        if (!frame_buffer.empty())
        {
            ...
            while (!frame_buffer.empty())
            {
                ...
                {
                    memcpy(right_context_data.data() + i * FEATURE_DIM, frame_buffer[current_chunk_size + i].data(),
                           FEATURE_DIM * sizeof(float));
                }
                ...
                // 移除已处理的帧
                std::vector<std::vector<float>> remaining(frame_buffer.begin() + current_chunk_size,
                                                          frame_buffer.end());
                frame_buffer = remaining;
            }
        }
...
bool save_npy(const std::string &filename, const std::vector<std::vector<float>> &features, size_t batch_size = 1)
{
    if (features.empty())
    {
    ...
    int header_len = snprintf(header, sizeof(header),
                              "{'descr': '<f4', 'fortran_order': False, 'shape': (%zu, %zu, %zu), }", batch_size, T, D);
...
extern "C" void *audio_callback(uint8_t *data, uint32_t len)
{
    ...
    while (g_audio_carryover.size() >= process_chunk_size)
    {
        ...
        int num_lfr = ctx.fbank_extractor->process_chunk(g_audio_carryover.data(), process_chunk_size, lfr_frames);
        if (num_lfr > 0)
        {
            for (const auto &frame : lfr_frames)
            {
                ctx.feature_queue->push(frame);
                if (ctx.total_frames_produced)
                    (*ctx.total_frames_produced)++;
            }
        }
    ...
    if (ctx.total_input_chunks)
        (*ctx.total_input_chunks)++;
...
int main(int argc, char *argv[])
{
    ...
    // 启动消费者线程（传入模型文件路径）
    std::thread consumer(consumer_thread, &feature_queue, &consumer_output_logits, &consumer_frame_count, model_file);
