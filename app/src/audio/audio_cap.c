...
CVI_S32 SAMPLE_AUDIO_RECORD_PCM_FORMAT_FILE(void *callback(uint8_t *data, uint32_t len))
{
    if (callback == NULL)
    {
        printf("[fatal error] callback is NULL,fuc:%s,line:%d\n", __func__, __LINE__);
        return -1;
    }
    ...
    while (running)
    {
        ...
        callback(stFrame.u64VirAddr[0], stFrame.u32Len * s32OutputChnCnt * 2);
    }
...
int audio_cap_main(void *callback(uint8_t *data, uint32_t len))
{
    ...
    SAMPLE_AUDIO_RECORD_PCM_FORMAT_FILE(callback);
    if (MSG_INIT)
    {
        CVI_MSG_Deinit();
        MSG_INIT = CVI_FALSE;
